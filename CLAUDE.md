...
- **Pane icons and the plugin name live in a resource bundle built by a script, not in `PluginResources.resx`.** The `[ViewPart(Icon = "...")]` attribute and the matching `<property name="Icon">` in `Supervertaler.Trados.plugin.xml` are *resource names*, resolved against `Supervertaler.Trados.plugin.resources`. Both places have to agree, the same way keyboard shortcuts do.
  - An SDK-style project **cannot** compile images into a `.resx`: `GenerateResource` fails with MSB3823/MSB3822 unless you set `GenerateResourceUsePreserializedResources` and reference `System.Resources.Extensions`. Do not take that route. The preserialized format needs that assembly loadable *when the resource is read*, which Studio does during plugin discovery - possibly before `AppInitializer` registers its `AssemblyResolve` hook. The failure mode is the plugin not loading, for every user, in exchange for an icon.
  - **The resources must be `System.Drawing.Icon`, from `.ico` files - never `Bitmap` from `.png`.** `Sdl.Desktop.Platform`'s `AbstractViewPart.Icon` is declared `System.Drawing.Icon`, and so is every one of Studio's own `ImageResources.*_Icon`. A bundle holding `Bitmap`s reads back perfectly outside Studio and then **kills the editor**: `NullReferenceException` in `TranslatableDocumentViewPart.GetControl()`, deep inside Studio's own tab machinery, with our panes simply absent. Cost a crash and an orphaned licence seat on 2026-09-09. `tools/make_pane_icons.py` writes both a `.png` (to look at) and a multi-resolution `.ico` (16/24/32, what ships).
  - Instead, `tools/make_plugin_resources.ps1` writes the classic format with `System.Resources.ResourceWriter` under **Windows PowerShell** (which is .NET Framework 4.8). It must be `powershell.exe`, never `pwsh` - PowerShell 7 is .NET 8 and writes the new format, reintroducing the problem silently.
  - The csproj generates it into `Resources/generated/` (gitignored) before build, embeds it with `LogicalName`, and copies it beside the DLL. Generating into `obj/` did not survive the build; do not move it back.
  - `PluginResources.resx` is kept as documentation only and is `Remove`d from `EmbeddedResource`. To change an icon or the plugin name, edit the **script**.
- **Studio validates a plugin's assembly references against a public-API whitelist, and refuses to load the plugin on any miss.** `Sdl.FileTypeSupport.Framework.Implementation.dll` is NOT public: a compile-time `<Reference>` to it produced *"Cannot load the plug-in … Assembly reference is not public"* at startup and the plugin did not load at all (2026-09-13, #132). The validator checks the manifest's references, not what runs — the assembly is loaded in Studio's process regardless — so a concrete type from it (e.g. `DocumentItemFactory`) is reached by name at runtime with `Activator.CreateInstance`, and never referenced. Before adding any `Sdl.*` reference, check it is one the existing csproj already uses or an `*Api` assembly.
...
- **Windows Defender silently kills long inline PowerShell that reflectively loads the built plugin.** The signature is `Trojan:Win32/ClickFix.IIN!MTB`, and it is a false positive — but the process is terminated *before it runs*, so the caller gets empty output rather than an error. That reads like "the method returned nothing", i.e. a code bug, and sends you debugging the wrong thing. It killed the figures smoke test four times in 93 seconds on 2026-09-07 (23:33–23:35), and had been doing it since at least 1 September.
  ...
  - **The trigger is the COMMAND LINE, not PowerShell.** Confirmed again on 2026-09-12 13:07: the script was already being run with `-File` and ran clean, but Defender killed the `bash.exe` that WROTE it, because the heredoc put the whole PowerShell body - `LoadFrom` and all - onto bash's own command line. Same silhouette, no PowerShell process involved. So `cat > .dev/x.ps1 <<'EOF'` is not a safe way to create these scripts. **Write them with a file-writing tool instead**, which presents no command line at all; keep heredocs for short, ordinary shell work.
  - **Second symptom to recognise: `EPERM: operation not permitted, uv_spawn 'bash.exe'`.** The documented symptom is empty output from a killed process; this is the other one, and it appears when Defender terminates bash as it starts. Both mean "look at Protection history", not "retry" and not "the code returned nothing". On 2026-09-12 it was read as a transient and worked around by accident.
