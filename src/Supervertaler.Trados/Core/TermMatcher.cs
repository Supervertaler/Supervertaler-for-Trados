    ...
    public class TermMatcher
    {
        ...
        internal static string NormalizeScriptChars(string text)
        {
            ...
            foreach (char c in text)
            {
                if ((c >= '\u2080' && c <= '\u2089') || c == '\u2070' ||
                    c == '\u00B9' || c == '\u00B2' || c == '\u00B3' ||
                    (c >= '\u2074' && c <= '\u2079') || IsScriptSign(c) || IsRadicalDot(c) || IsSpaceVariant(c) || IsApostropheVariant(c))
                {
                    hasScript = true;
                    break;
                }
            ...
            foreach (char c in text)
            {
                ...
                else if (IsRadicalDot(c))                 sb.Append('\u00B7'); // OH∙ / OH· / OH⋅ - one dot for matching
        ...
        /// matching so a term saved from one document matches the next.
        /// </summary>
        internal static bool IsRadicalDot(char c)
        {
            return c == '\u00B7' || c == '\u2219' || c == '\u22C5' || c == '\u2022';
        }
